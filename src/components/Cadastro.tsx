    ...
    const cadastrarUsuario = (e: FormEvent) => {
        ...
        axios.post('http://10.137.9.136:8000/api/store',
            ...
            if (response.data.success === false) {
                ...
                if("passsword" in response.data.error){
                        ...
                        <div className='card-body'>
                            ...
                            <form onSubmit={cadastrarUsuario} className='row g-3'>
                                <div className='col-6'>
                                    ...
                                    <input type="text" name='nome' className={'form-control' + (nomeErro ? ' border-danger' : '')} required onChange={handleState} />
                                    <div className="text-danger">{nomeErro}</div>
                                </div>
                                <div className='col-6'>
                                    <label htmlFor="email" className='form-label' >E-mail</label>
                                    <input type="text" name='email' className={'form-control' + (emailErro ? ' border-danger' : '')} required onChange={handleState} />
                                    <div className="text-danger">{emailErro}</div>
                                </div>
                                <div className='col-4'>
                                    <label htmlFor="cpf" className='form-label'>CPF</label>
                                    <input type="text" name='cpf' className={'form-control' + (cpfErro ? ' border-danger' : '')} required onChange={handleState} />
                                    <div className="text-danger">{cpfErro}</div>
                                </div>
                                <div className='col-4'>
                                    <label htmlFor="password" className='form-label'>Senha</label>
                                    <input type="text" name='password' className={'form-control' + (passwordErro ? ' border-danger' : '')} required onChange={handleState} />
